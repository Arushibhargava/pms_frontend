import { Send, Users, Bell, CheckCircle, AlertCircle, MessageSquare, Sparkles } from 'lucide-react';
...
const Cnotification = () => {
 const [formData, setFormData] = useState({
  ...
  'https://pms-backend-00j9.onrender.com/api/coordinator/fetch_users/',
  {
    params: { user_type: "coordinator" },
    headers: {
...
setTeamMembers(members);
        ...
        console.error("Failed to fetch assigned mentor/coordinator:", error);
      }
    };
  ...
  try {
    ...
    if (formData.recipient === "all_teams") {
      payload.target = "all_teams";
    }
    else if (formData.recipient === "all_mentors") {
      payload.target = "all_mentors";
    }
    else {
      payload.target = "single";
      payload.receiver = formData.recipient;
    }

    const response = await axios.post(
      'https://pms-backend-00j9.onrender.com/api/coordinator/sendnotification/',
      payload,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`
