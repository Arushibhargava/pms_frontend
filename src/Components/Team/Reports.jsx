...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchAssignedUsers = async () => {
      try {
        const res = await axios.get('https://pms-backend-00j9.onrender.com/api/team/assigned-users/', {
          headers: {
        ...
        const members = [];
        if (res.data.mentor) members.push(res.data.mentor);
        if (res.data.coordinator) members.push(res.data.coordinator);
                  ...
                  >
                    ...
                    {teamMembers.map((member) => (
                      <option key={member.username} value={member.username}>
                        {member.name} ({member.user_type.charAt(0).toUpperCase() + member.user_type.slice(1)})
                      </option>
                    ))}
                  </select>
                </div>
